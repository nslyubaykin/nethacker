...
class Agent:
    ...
    def emergency_strategy(self):
        ...
        # hypothesis: conserving identified healing until an enemy is actually
        # threatening us, while using it before the usual critical-health point,
        # prevents fatal melee exchanges without spending recovery potions after
        # the fight has already ended.
        threatened = any(dis <= 2 for dis, *_ in self.get_visible_monsters())
        if (
                (self.blstats.hitpoints < 1 / 3 * self.blstats.max_hitpoints or
                 (threatened and self.blstats.hitpoints < 2 / 5 * self.blstats.max_hitpoints)
